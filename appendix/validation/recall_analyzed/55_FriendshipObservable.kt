...
val friendshipObservable by instance<FriendshipObservable>() //#property_delegation,inference
...
class FriendshipObservable {
    ...
    private val cd = CompositeDisposable() //#inference

    infix fun addFriendshipListener(friendshipListener: FriendshipListener) = with (fbReader) { //#infix_function,lambda
        cd.addAll(
                friendshipRequestedListener()
                        .subscribe { friendshipListener.onPeerRequestedFriendship(it) }, //#lambda
                friendshipAcceptedListener()
                        .subscribe { friendshipListener.onPeerAcceptedFriendship(it) }, //#lambda
                friendshipDeletedListener()
                        .subscribe { friendshipListener.onPeerDeletedFriendship(it) }) //#lambda
    }
