    ...
    override fun initActivity() {
        ...
        if (config.wereFavoritesMigrated) {
            ...
            if (config.appRunCount == 0) {
                ...
                config.wereFavoritesMigrated = true
                ensureBackgroundThread { //#lambda
                    val favorites = ArrayList<Favorite>() //#inference
                    val favoritePaths = mediaDB.getFavorites().map { it.path }.toMutableList() as ArrayList<String> //#inference,lambda
                    favoritePaths.forEach { //#lambda
