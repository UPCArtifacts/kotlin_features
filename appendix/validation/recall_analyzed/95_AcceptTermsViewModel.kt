...
class AcceptTermsViewModel : ViewModel() {
    ...
    fun markTermAsAccepted(url: String, accepted: Boolean) {
        termsList.value?.invoke()?.map { //#safe_call,lambda,safe_call
            if (it.url == url) {
                it.copy(accepted = accepted) //#func_call_with_named_arg
            } else it
        }?.let { //#lambda
    ...
    fun initSession(session: MXSession?) {
        mxSession = session
        termsManager = mxSession?.termsManager //#safe_call
    }

    fun acceptTerms() {
        val acceptedTerms = termsList.value?.invoke() ?: return //#inference,safe_call

        acceptTerms.postValue(MxAsync.Loading())
        val agreedUrls = acceptedTerms.map { it.url } //#inference,lambda

        termsManager?.agreeToTerms(termsArgs.type, //#safe_call
                termsArgs.baseURL,
                agreedUrls,
                termsArgs.token,
                object : ApiCallback<Unit> { //#singleton
                    ...
                    override fun onMatrixError(e: MatrixError?) {
                        acceptTerms.postValue(MxAsync.Error(R.string.unknown_error))
                        Log.e(LOG_TAG, "Failed to agree to terms " + e?.message) //#safe_call
                    }
                }
        )
    }
    ...
    fun loadTerms(preferredLanguageCode: String) {
        ...
        termsManager?.get(termsArgs.type, termsArgs.baseURL, object : ApiCallback<GetTermsResponse> { //#safe_call,singleton
            override fun onSuccess(info: GetTermsResponse) {

                val terms = info.serverResponse.getLocalizedTerms(preferredLanguageCode).map { //#inference,lambda
                    Term(it.localizedUrl ?: "", //#func_call_with_named_arg
                            it.localizedName ?: "", 
                            it.version,
    ...
    companion object { //#companion
        private val LOG_TAG = AcceptTermsViewModel::javaClass.name //#inference
    }

}

data class Term( //#data_class,func_with_default_value
        val url: String,
        val name: String,
