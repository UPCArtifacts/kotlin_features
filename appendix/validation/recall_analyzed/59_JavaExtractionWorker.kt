...
class JavaExtractionWorker(context: Context, data: Data) : BaseDecompiler(context, data) {
    ...
    private fun decompileWithCFR(jarInputFiles: File, javaOutputDir: File) {
        cleanMemory()
        val jarFiles = jarInputFiles.listFiles() //#inference
        val options = mapOf<String, String>( //#inference
            "outputdir" to javaOutputDir.canonicalPath,
            "lomem" to "true"
        )
        val cfrDriver = CfrDriver.Builder().withOptions(options).build() //#inference
        cfrDriver.analyse(jarFiles.map { it.canonicalPath }) //#lambda
    }
    ...
    private fun decompileWithFernFlower(jarInputFiles: File, javaOutputDir: File) {
        ...
        javaOutputDir.listFiles().forEach { decompiledJarFile -> //#lambda
            if (decompiledJarFile.exists() && decompiledJarFile.isFile && decompiledJarFile.extension == "jar") {
                ZipUtils.unzip(decompiledJarFile, javaOutputDir, printStream!!) //#unsafe_call
    ...
    override fun doWork(): ListenableWorker.Result {
        ...
        context.getString(R.string.decompilingToJava).let { //#lambda
        ...
        val sourceInfo = SourceInfo.from(workingDirectory) //#inference
            .setPackageLabel(packageLabel)
            .setPackageName(packageName)
            .persist()

        try {
            when (decompiler) { //#when_expr
