...
class CheckmarkRepositoryTest() {
    @Test
    fun testCRUD() = asyncTest { //#lambda
        val db = DependencyResolver.getDatabase() //#inference

        val habitA = 10 //#inference
        var checkmarksA = listOf(Checkmark(LocalDate(2019, 1, 15), 100), //#inference
                                 Checkmark(LocalDate(2019, 1, 7), 500),
                                 Checkmark(LocalDate(2019, 1, 1), 900))

        val habitB = 35 //#inference
        val checkmarksB = listOf(Checkmark(LocalDate(2019, 1, 30), 50), //#inference
        ...
        val repository = CheckmarkRepository(db) //#inference
