...
import kotlinx.coroutines.CompletableDeferred //#coroutine

typealias BiometricDeferred = CompletableDeferred<BiometricPrompt.CryptoObject?> //#type_alias

/**
 * Container for [BiometricPrompt]
 * Inspired by coroutine's CommonPool
 */
object BiometricUtils { //#singleton
    ...
    private var lastUnlockTime = -1L //#inference

    private const val UNLOCK_TIME_INTERVAL = 15 * 60 * 1000 //#inference
    ...
    fun isSupported(context: Context): Boolean {
        ...
        val fingerprintManager = //#inference
    ...
    private fun getOrCreatePoolSync(): Executor =
        pool ?: Executors.newSingleThreadExecutor().also { pool = it } //#lambda
     ...
     */
    fun authenticate( //#func_with_default_value
        ...
        val info = BiometricPrompt.PromptInfo.Builder() //#inference
            .setTitle(activity.string(R.string.biometrics_prompt_title))
            .setNegativeButtonText(activity.string(R.string.kau_cancel))
            .build()
        val prompt = BiometricPrompt(activity, executor, Callback(activity, deferred)) //#inference
