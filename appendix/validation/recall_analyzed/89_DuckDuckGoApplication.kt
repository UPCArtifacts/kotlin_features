...
open class DuckDuckGoApplication : HasAndroidInjector, Application(), LifecycleObserver {
    ...
    override fun onCreate() {
        ...
        ProcessLifecycleOwner.get().lifecycle.also { //#lambda
            it.addObserver(this)
    ...
    private fun configureUncaughtExceptionHandler() {
        ...
        RxJavaPlugins.setErrorHandler { throwable -> //#lambda
            if (throwable is UndeliverableException) { //#smart_cast
    ...
    private fun initializeHttpsUpgrader() {
        thread { httpsUpgrader.reloadData() }  //#lambda
    }

    private fun submitUnsentFirePixels() {
        val count = unsentForgetAllPixelStore.pendingPixelCountClearData //#inference
        Timber.i("Found $count unsent clear data pixels") //#string_template
        if (count > 0) {
            val timeDifferenceMillis = System.currentTimeMillis() - unsentForgetAllPixelStore.lastClearTimestamp //#inference
            if (timeDifferenceMillis <= APP_RESTART_CAUSED_BY_FIRE_GRACE_PERIOD) {
                Timber.i("The app was re-launched as a result of the fire action being triggered (happened ${timeDifferenceMillis}ms ago)") //#string_template
                launchedByFireAction = true
            }
            for (i in 1..count) { //#range_expr
    ...
    private fun configureDataDownloader() {
        appConfigurationSyncer.scheduleImmediateSync()
            ...
            .doAfterTerminate { //#lambda
                appConfigurationSyncer.scheduleRegularSync()
            }
            .subscribe({}, { Timber.w("Failed to download initial app configuration ${it.localizedMessage}") }) //#lambda,string_template
    }
